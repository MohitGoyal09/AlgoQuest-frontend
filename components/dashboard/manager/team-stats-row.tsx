...
export function TeamStatsRow({ metrics }: TeamStatsProps) {
  ...
  if (!metrics) {
    return (
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        {[1, 2, 3, 4].map((i) => (
          <Card key={i} className="bg-[#0b101b] border-white/5">
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
      <Card className="bg-[#0b101b] border-white/5 shadow-lg relative overflow-hidden group">
        ...
        <CardContent className="pt-6">
          ...
          <div className="text-3xl font-bold text-white mt-2">{metrics.total_members}</div>
      ...
      <Card className="bg-[#0b101b] border-white/5 shadow-lg group">
        <CardContent className="pt-6">
           <div className="flex justify-between items-start">
              <div>
                <div className="text-sm font-medium text-muted-foreground">Burnout Risk</div>
                <div className="text-3xl font-bold text-white mt-2">{metrics.burnout_risk}%</div>
              </div>
              <div className={`p-2 rounded-lg ${metrics.burnout_risk > 50 ? 'bg-red-500/20 text-red-400' : 'bg-green-500/20 text-green-400'}`}>
                 <AlertTriangle className="w-5 h-5" />
              </div>
           </div>
           <Progress value={metrics.burnout_risk} className="h-1 mt-4 bg-slate-800" indicatorClassName={metrics.burnout_risk > 50 ? 'bg-red-500' : 'bg-green-500'} />
        </CardContent>
      </Card>
        ...
        <CardContent className="pt-6">
           <div className="flex justify-between items-start">
              <div>
                ...
                <div className="text-3xl font-bold text-white mt-2">{metrics.avg_velocity.toFixed(1)}</div>
              </div>
